...
<html>
  ...
  <body>
    <p class="nbperso">Nombre de personnages créés : <?= $manager->count() ?></p>
<?php
...
?>
    ...
    <div class="cadre">
        <fieldset>
            ...
            <p style="color: white">
                ...
                Nom : <span class="myname"> <?= htmlspecialchars($perso->nom()) ?></span><br />
                Dégâts subits : <?= $perso->degats() ?><br />
    ...
    <div class="combat">
        <fieldset>
            <legend class="attack">Qui attaquer ?</legend>
            <p>
                    ...
                    else
                    {
                      foreach ($retourPersos as $unPerso) {
                        ...
                        <?php if ($unPerso->type() == 'guerrier'){
                            ...
                            echo 'card guerrier'; }
                            else if ($unPerso->type() == 'magicien'){
                            echo 'card magicien'; }
                            else{
                            echo 'card archer';} ?>">
          
                                    <div class="card-body">
                                        <!--<h5 class="card-title">qui attaquer ?</h5>-->
                                        <p class="card-text"><a class="stylea btn btn-primary" href="?frapper= <?= $unPerso->id() ?>"> <?= htmlspecialchars($unPerso->nom()) ?></a> (dégâts : <?= $unPerso->degats() ?> | type : <?= $unPerso->type()?>)</p>
